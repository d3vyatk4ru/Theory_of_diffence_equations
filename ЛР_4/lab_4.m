function [] = lab_4 ()
    ...
    % Руками выбираем только одно положение равновесия, так как
    % второе является седлом и его рассматривать не нужно.
...
function [value, isterminal, direction] = stopInt(t, X)
    ...
    % + delta *(t < 1) --- для того чтобы не было остановки интегрирования
    % на 1-2 шаге, в случае близости к ПР.
    value = X(2) - equilibriumY + delta *(t < 0.1);
    
    % Останавливаем расчёт при условии, если пересекаем ось Х снизу вверх.
    % При -1 остановка расчёта при пересечении сверху вниз оси Х.
    isterminal = 1;    
    direction = 1;
